...
El backend soporta autenticación con certificado digital. En modo normal
(HTTP, puerto 8080) está **desactivado**; se habilita con el perfil `cert`,
que arranca el servidor en **HTTPS con TLS mutuo (mTLS)** en el puerto **8443**.
...
## 2. Arrancar el backend en modo certificado (HTTPS 8443)

```powershell
$env:DB_PASSWORD="1234"
.\mvnw.cmd -DskipTests spring-boot:run "-Dspring-boot.run.jvmArguments=-Dspring.profiles.active=cert"
```
...
- `application-cert.properties` — SSL, truststore y `server.ssl.client-auth=want`
